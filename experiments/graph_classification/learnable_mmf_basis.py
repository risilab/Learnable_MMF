...
def _parse_args():
    ...
    parser.add_argument('--drop', '-drop', type = int, default = 1, help = 'Number of columns dropped each rotation')
    parser.add_argument('--dim', '-dim', type = int, default = 2, help = 'Dimension left at the end')
    parser.add_argument('--epochs', '-epochs', type = int, default = 1024, help = 'Number of epochs')
...
for sample in range(num_data):
    ...
    if L > 0:
        if drop == 1:
            wavelet_indices, rest_indices = heuristics_k_neighbors_single_wavelet(A.to_sparse(), L = L, K = K, drop = drop, dim = dim)
        else:
            wavelet_indices, rest_indices = heuristics_k_neighbors_multiple_wavelets(A.to_sparse(), L = L, K = K, drop = drop, dim = dim)

        A_rec, U, D, mother_coefficients, father_coefficients, mother_wavelets_, father_wavelets_ = learnable_mmf_train(
                A, L = L, K = K, drop = drop, dim = dim, wavelet_indices = wavelet_indices, rest_indices = rest_indices, epochs = epochs, learning_rate = learning_rate, early_stop = True)
